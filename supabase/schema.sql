...
alter table public.profiles add column if not exists livekit_token text;
alter table public.profiles add column if not exists livekit_token_expires_at timestamptz;
alter table public.profiles add column if not exists livekit_identity text;
...
-- Reads the "username" passed in signUp() options.data.username, falling
-- back to the email prefix if not provided.
...
-- ---------- FRIENDS ----------
-- one row per relationship; user_id = requester, friend_id = addressee
do $$
begin
...
begin
  if not exists (select 1 from information_schema.tables where table_name = 'calls' and table_schema = 'public') then
    create table public.calls (
      ...
      constraint calls_have_target check (receiver_id is not null or group_id is not null)
    );
  end if;
end $$;
...
create index if not exists calls_receiver_idx
  on public.calls (receiver_id, status, created_at);

create index if not exists calls_group_idx
  on public.calls (group_id, status, created_at);
  ...
  create policy "Call participants can view calls"
    ...
    using (
      ...
      or exists (
        select 1
        from public.group_members gm
        where gm.group_id = calls.group_id
          and gm.profile_id = auth.uid()
      )
    );
exception when duplicate_object then
...
-- Enable realtime updates for the messages and profiles tables
